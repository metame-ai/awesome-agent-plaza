# Embodied Agents

- [Embodied Agents](#embodied-agents) 
	- [Survey](#survey)
	- [VLA](#vla)
	- [Embodied Agents](#embodied-agents-1)
	- [Data](#data)
	- [Toolkits](#toolkits)
	- [Misc](#misc)

## Survey


## VLA
- **RLinf-VLA: A Unified and Efficient Framework for VLA+RL Training**, `arXiv, 2510.06710`, [arxiv](http://arxiv.org/abs/2510.06710v1), [pdf](http://arxiv.org/pdf/2510.06710v1.pdf), cication: [**-1**](None) 

	 *Hongzhi Zang, Mingjie Wei, Si Xu, ..., Chao Yu, Yu Wang*
- **VLA-R1: Enhancing Reasoning in Vision-Language-Action Models**, `arXiv, 2510.01623`, [arxiv](http://arxiv.org/abs/2510.01623v1), [pdf](http://arxiv.org/pdf/2510.01623v1.pdf), cication: [**-1**](None) 

	 *Angen Ye, Zeyu Zhang, Boyuan Wang, ..., Dapeng Zhang, Zheng Zhu*
- **From Spatial to Actions: Grounding Vision-Language-Action Model in 
  Spatial Foundation Priors**, `arXiv, 2510.17439`, [arxiv](http://arxiv.org/abs/2510.17439v1), [pdf](http://arxiv.org/pdf/2510.17439v1.pdf), cication: [**-1**](None) 

	 *Zhengshen Zhang, Hao Li, Yalun Dai, ..., Xinghang Li, Pan Zhou*
- **Spatial Forcing: Implicit Spatial Representation Alignment for 
  Vision-language-action Model**, `arXiv, 2510.12276`, [arxiv](http://arxiv.org/abs/2510.12276v2), [pdf](http://arxiv.org/pdf/2510.12276v2.pdf), cication: [**-1**](None) 

	 *Fuhao Li, Wenxuan Song, Han Zhao, ..., Long Zeng, Haoang Li*


## Embodied Agents

- **Green-VLA: Staged Vision-Language-Action Model for Generalist Robots**, `arXiv, 2602.00919`, [arxiv](https://arxiv.org/abs/2602.00919v1), [pdf](https://arxiv.org/pdf/2602.00919v1.pdf), cication: [**-1**](None) 

	 *I. Apanasevich, M. Artemyev, R. Babakyan, ..., S. Sobolnikov, A. Postnikov*
  Embodied AI**, `arXiv, 2510.05684`, [arxiv](http://arxiv.org/abs/2510.05684v1), [pdf](http://arxiv.org/pdf/2510.05684v1.pdf), cication: [**-1**](None) 

	 *Suwhan Choi, Jaeyoon Jung, Haebin Seong, ..., Youngjae Yu, Yunsung Lee*
- **Robot Learning: A Tutorial**, `arXiv, 2510.12403`, [arxiv](http://arxiv.org/abs/2510.12403v1), [pdf](http://arxiv.org/pdf/2510.12403v1.pdf), cication: [**-1**](None) 

	 *Francesco Capuano, Caroline Pascal, Adil Zouitine, ..., Thomas Wolf, Michel Aractingi*


## Data


## Toolkits


## Misc