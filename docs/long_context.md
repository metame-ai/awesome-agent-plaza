# Long Context

- [Long Context](#long-context) 
	- [Survey](#survey)
	- [Long Context](#long-context-1)
	- [Linear Attention](#linear-attention)
	- [Text Visual Encoding](#text-visual-encoding)
	- [Context Engineering](#context-engineering)
	- [Toolkits](#toolkits)
	- [Misc](#misc)

## Survey


## Long Context
- **R-Horizon: How Far Can Your Large Reasoning Model Really Go in Breadth 
  and Depth?**, `arXiv, 2510.08189`, [arxiv](http://arxiv.org/abs/2510.08189v2), [pdf](http://arxiv.org/pdf/2510.08189v2.pdf), cication: [**-1**](None) 

	 *Yi Lu, Jianing Wang, Linsen Guo, ..., Wei Wang, Xunliang Cai* · ([reasoning-horizon.github](https://reasoning-horizon.github.io/))
- [Recursive Language Models](https://alexzhang13.github.io/blog/2025/rlm/) 

	 · ([𝕏](https://x.com/a1zhang/status/1978469116542337259))
- **Efficient Long-context Language Model Training by Core Attention 
  Disaggregation**, `arXiv, 2510.18121`, [arxiv](http://arxiv.org/abs/2510.18121v1), [pdf](http://arxiv.org/pdf/2510.18121v1.pdf), cication: [**-1**](None) 

	 *Yonghao Zhuang, Junda Chen, Bo Pang, ..., Eric Xing, Hao Zhang*
- **Every Attention Matters: An Efficient Hybrid Architecture for 
  Long-Context Reasoning**, `arXiv, 2510.19338`, [arxiv](http://arxiv.org/abs/2510.19338v2), [pdf](http://arxiv.org/pdf/2510.19338v2.pdf), cication: [**-1**](None) 

	 *Ling Team, Bin Han, Caizhi Tang, ..., Zixuan Cheng, Jun Zhou*
- **LoongRL: Reinforcement Learning for Advanced Reasoning over Long 
  Contexts**, `arXiv, 2510.19363`, [arxiv](http://arxiv.org/abs/2510.19363v2), [pdf](http://arxiv.org/pdf/2510.19363v2.pdf), cication: [**-1**](None) 

	 *Siyuan Wang, Gaokai Zhang, Li Lyna Zhang, ..., Dongyao Chen, Mao Yang*
- **LongCodeZip: Compress Long Context for Code Language Models**, `arXiv, 2510.00446`, [arxiv](http://arxiv.org/abs/2510.00446v1), [pdf](http://arxiv.org/pdf/2510.00446v1.pdf), cication: [**-1**](None) 

	 *Yuling Shi, Yichun Qian, Hongyu Zhang, ..., Beijun Shen, Xiaodong Gu*


## Linear Attention
- [Higher-order Linear Attention](https://yifanzhang-pro.github.io/HLA/) 

	 · ([𝕏](https://x.com/yifan_zhang_/status/1984099671657304207))


## Text Visual Encoding
- **Text or Pixels? It Takes Half: On the Token Efficiency of Visual Text 
  Inputs in Multimodal LLMs**, `arXiv, 2510.18279`, [arxiv](http://arxiv.org/abs/2510.18279v2), [pdf](http://arxiv.org/pdf/2510.18279v2.pdf), cication: [**-1**](None) 

	 *Yanhong Li, Zixuan Lan, Jiawei Zhou*
- [A more serious thread on the DeepSeek-OCR hype / serious misinterpretation going on.](https://x.com/Kangwook_Lee/status/1980709454522744902)  𝕏 
- **PixelWorld: How Far Are We from Perceiving Everything as Pixels?**, `arXiv, 2501.19339`, [arxiv](http://arxiv.org/abs/2501.19339v3), [pdf](http://arxiv.org/pdf/2501.19339v3.pdf), cication: [**-1**](None) 

	 *Zhiheng Lyu, Xueguang Ma, Wenhu Chen* · ([𝕏](https://x.com/ZhihengLyu/status/1886974422789099664))


## Context Engineering
- **AgentFold: Long-Horizon Web Agents with Proactive Context Management**, `arXiv, 2510.24699`, [arxiv](http://arxiv.org/abs/2510.24699v1), [pdf](http://arxiv.org/pdf/2510.24699v1.pdf), cication: [**-1**](None) 

	 *Rui Ye, Zhongwang Zhang, Kuan Li, ..., Jingren Zhou, Yong Jiang*
- 🌟 **Agentic Context Engineering: Evolving Contexts for Self-Improving 
  Language Models**, `arXiv, 2510.04618`, [arxiv](http://arxiv.org/abs/2510.04618v1), [pdf](http://arxiv.org/pdf/2510.04618v1.pdf), cication: [**-1**](None) 

	 *Qizheng Zhang, Changran Hu, Shubhangi Upasani, ..., James Zou, Kunle Olukotun* · ([𝕏](https://x.com/rryssf_/status/1976269613072843063))
- **LightMem: Lightweight and Efficient Memory-Augmented Generation**, `arXiv, 2510.18866`, [arxiv](http://arxiv.org/abs/2510.18866v1), [pdf](http://arxiv.org/pdf/2510.18866v1.pdf), cication: [**-1**](None) 

	 *Jizhan Fang, Xinle Deng, Haoming Xu, ..., Huajun Chen, Ningyu Zhang*


## Toolkits


## Misc
